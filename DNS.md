...
- **Local zone:** ~20 A records under `greenbean.org`, scripted from a single `inventory.yaml` — see [§9](#9-local-zone-sync-inventoryyaml--pi-hole)
- **Upstreams:** local recursive, validating Unbound on each host — see [UNBOUND.md](https://github.com/greenbeanorg/homelab-docs/blob/main/UNBOUND.md)
- **Date:** August 2026
...
(two upgrade paths, two ways for config to drift), not a feature. See [§7](#7-known-limitations).
...
install, and cutover procedure: [UNBOUND.md](https://github.com/greenbeanorg/homelab-docs/blob/main/UNBOUND.md).
...
document: [ODROID-XU4.md](https://github.com/greenbeanorg/homelab-docs/blob/main/ODROID-XU4.md).
...
- Image tag **pinned** (Pi-hole ships date-based tags like `2026.07.2`, not
  semver) via `PIHOLE_TAG=` in a `.env` beside the compose file. `latest` on
  the network's primary resolver is an unscheduled outage waiting for a bad
  push.
- Web UI HTTPS-only on `443`, self-signed cert generated at first start
- Upstreams: local Unbound at `127.0.0.1#5335` — recursive and
  DNSSEC-validating, not forwarded to a third party. Build and cutover:
  [UNBOUND.md](https://github.com/greenbeanorg/homelab-docs/blob/main/UNBOUND.md).
...
> the UI will appear to work and then silently revert on the next
> `docker compose up`. If a setting must persist, either set it in the
> environment or keep it out of the environment entirely — never both.

**Routine update** (this is the primary resolver — see [§7](#7-known-limitations) before running it unattended):
...
blocking lost when [Unbound](https://github.com/greenbeanorg/homelab-docs/blob/main/UNBOUND.md)
replaced Quad9 as upstream — see [§7](#7-known-limitations). HaGeZi's own docs
flag the full TIF list as needing ≥1GB RAM; confirmed safe on the XU4 (232MB /
1.94GB container usage, 1.7GB free at the host level, no swap in use). A
companion newly-registered-domains list (`nrd-1m.txt`) was considered and
deliberately **not** added — it flags freshly registered domains on sight,
which is a poor fit for a household that registers its own domains regularly.
...
For routine re-sync of the **local zone** specifically — as opposed to
blocklists/settings, which stay a Teleporter job — see
[§9](#9-local-zone-sync-inventoryyaml--pi-hole).
...
This section is the procedure. The outstanding-work note lives in [§7](#7-known-limitations).
...
stub and hand edits are **silently reverted on reboot** — the real
configuration lives in netplan or NetworkManager. Auditing with `cat` will
produce a list of hosts that look correct and aren't.
...
dead primary yields *slow but working* resolution fleet-wide, which is harder
to diagnose than an outright failure because nothing errors.
...
> Resolver configuration managed fleet-wide means the next renumber —
> including retiring the flat `10.x.x.0/24` — is a variable change and a
> playbook run rather than another manual sweep with the same class of misses.
...
**The local zone was hand-maintained; now scripted, still not
version-controlled.** `inventory.yaml` → Pi-hole sync ([§9](#9-local-zone-sync-inventoryyaml--pi-hole))
closes the "forgot to update the second resolver" failure mode, but the sync
is push-only — a record added by hand directly in the Pi-hole UI has no path
back into `inventory.yaml`, and will be silently deleted on the next
`pihole_repave.py --apply`. The zone's source of truth is a YAML file on
`swearengen`, not this repo.

**Monitoring covers up/down, not correctness.** Uptime Kuma watches both hosts
(see [UPTIME-KUMA.md](https://github.com/greenbeanorg/homelab-docs/blob/main/UPTIME-KUMA.md)),
but a Pi-hole that is up and returning NXDOMAIN for everything looks healthy.
A DNS-query-type monitor against a known record on each resolver is the right
check here.

**Recursive resolution traded one risk for another.** Moving off Quad9 to
per-host Unbound ([UNBOUND.md](https://github.com/greenbeanorg/homelab-docs/blob/main/UNBOUND.md))
removes a third party from every query, but each resolver can now fail in a
way Pi-hole's own health check won't see — a stale DNSSEC trust anchor or
corrupt root hints leaves the container reporting healthy while nothing
resolves. Periodic re-verification (`dig dnssec-failed.org` expecting
`SERVFAIL`, on both hosts) is the mitigation; there is no equivalent automatic
check for this today.
...
2. **Rebuild the secondary** (fastest path — an LXC from template, then the
   ...
   ([UNBOUND.md §2–4](https://github.com/greenbeanorg/homelab-docs/blob/main/UNBOUND.md))
   **before** importing config — a resolver pointed at a not-yet-working
   Unbound is worse than one still on `9.9.9.9`. Import the most recent
   Teleporter archive, then flip the upstream.
3. **Point option 6 at the secondary only**, renew a client, confirm
   resolution and local records.
4. **Rebuild the primary** (Armbian image → Docker → compose file with the
   pinned tag → Teleporter import). Same order: Unbound installed and
   validated standalone first, upstream flipped only after it passes.
...
## 9. Local zone sync — `inventory.yaml` → Pi-hole

Partial answer to the hand-maintained-zone gap flagged in [§7](#7-known-limitations):
the ~20 local A records are now derived from a single YAML inventory and
pushed to both resolvers by script rather than clicked in twice through the
UI. **Not committed to this repo** — the tooling lives locally
(`~/src/pihole_vlan_sync` on `swearengen`), since it embeds real lab IPs and
isn't worth building a sanitizer pass for two small scripts.

Two tools, deliberately different philosophies:

| | `pihole_vlan_sync.py` | `pihole_repave.py` |
|---|---|---|
| Model | Incremental — diffs, only touches drift | Declarative — wipes everything, rebuilds from inventory |
| Preserves unmanaged records | Yes | **No — deletes anything not in inventory.yaml** |
| Use for | One-off fixes, routine re-sync | Normal day-to-day sync |

`inventory.yaml` is the same file `kuma_sync.py` reads (site → category →
monitor list — see
[UPTIME-KUMA.md](https://github.com/greenbeanorg/homelab-docs/blob/main/UPTIME-KUMA.md)) —
one source of truth for both monitoring targets and DNS records, not a second
file to keep in sync by hand. Each entry produces two Pi-hole records:
`<slug>.greenbean.org` and bare `<slug>`, both pointed at whatever IP is
embedded in that entry's `target` (a bare host for `ping` monitors, parsed out
of the URL for `http`/`json-query` monitors).

```
python pihole_repave.py                        # dry run — always first
python pihole_repave.py --apply --yes-i-mean-it # wipe + rebuild both resolvers
```

The dry run prints an **orphan report** — every existing record
`inventory.yaml` doesn't know about — every time, including in dry-run,
because a wipe is unrecoverable and this is the only warning before it
happens.

> **Gotcha — Pi-hole app passwords can't do destructive API calls (open
> upstream bug).** Confirmed against v6.4.3: a session authenticated with a
> generated **app password** can `GET` fine but gets `403` on every
> `PUT`/`DELETE`, even with **Permit destructive actions via API** enabled in
> Settings. The same write succeeds instantly with the plain **admin
> password** (`pihole setpasswd`). Until upstream fixes it, both scripts use
> the admin password via `PIHOLE_PW_<host>` env vars, not a scoped app
> password — a real tradeoff (broader credential in a script) accepted for a
> two-box homelab only you run.

> **Gotcha — Pi-hole's local DNS records support multiple aliases per line,
> like `/etc/hosts`.** `10.x.x.1 opnsense.greenbean.org opnsense` is *one*
> stored entry, not two. A naive parser that splits on the first space only
> sees `"opnsense.greenbean.org opnsense"` as a single giant hostname, never
> recognizes either alias as already present, and pushes redundant duplicate
> lines on every run. Both scripts split on full whitespace and treat every
> alias on a line independently. `pihole_repave.py` sidesteps the problem
> entirely by not caring about the old format at all — one alias per line,
> always.
...
| Config sync (blocklists/settings)    | Web UI → Settings → Teleporter (export primary → import secondary)                                                        |
| Local zone sync (dry run)            | `python pihole_repave.py` (on `swearengen`, `~/src/pihole_vlan_sync`)                                                     |
| Local zone sync (apply)              | `python pihole_repave.py --apply --yes-i-mean-it`                                                                         |
| Upstream (both)                      | `127.0.0.1#5335` — local Unbound, not Quad9. Build/cutover: [UNBOUND.md](https://github.com/greenbeanorg/homelab-docs/blob/main/UNBOUND.md) |
