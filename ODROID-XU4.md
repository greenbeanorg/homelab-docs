# ODROID-XU4 — Orphaned Vendor Kernel → Armbian

**Status:** Complete
**Host:** `pihole` — ODROID-XU4, primary network-wide DNS
**Date:** August 2026

Rebuild of the ODROID-XU4 from a Hardkernel-kernel Ubuntu install onto a maintained
Armbian stack. The board had been installed as **Ubuntu 22.04**, later upgraded in place
to **24.04**, but the kernel and boot environment stayed Hardkernel's throughout — and
that vendor kernel had gone orphaned upstream.

This document covers **why a normal `do-release-upgrade` was rejected** as much as how the
board was reflashed. The decision is the interesting part; the flashing is four steps.

Companion document: [DNS.md](DNS.md) — the redundant Pi-hole design this host serves.
...
Installed as Ubuntu 22.04 (jammy) and upgraded in place to 24.04 (noble). The userspace
moved; the kernel and boot stack did not.
...
board-specific kernel and boot environment, which `do-release-upgrade` would have carried
forward untouched.
...
**The kernel existed only as a locally installed package** — the `100` pin is
`/var/lib/dpkg/status`, meaning no repository could supply a successor. No package holds
were in place (`apt-mark showhold` returned nothing), so this was orphanhood, not
configuration.
...
`deb http://deb.odroid.in/5422-s/ focal main` — a leftover from the original 22.04-era
install, disabled during one of the upgrades and never replaced.

So the installed system was: **Ubuntu 24.04 userspace + Hardkernel 5422 kernel + ODROID
boot configuration** — not an Ubuntu-native kernel/boot stack, and with no upstream path
for the half that mattered.
...
files live on the FAT partition at `/media/boot`. Editing `/boot` on an ODROID accomplishes
nothing and produces very confusing debugging sessions.
...
**U-Boot loads fixed filenames.** Changing kernels is therefore a boot-partition operation,
not a package operation.
...
With the ODROID kernel orphaned, a third in-place release upgrade risked leaving
**Ubuntu 26.04 userspace on an unmaintained 2024 kernel** — the same trap the 22.04 → 24.04
upgrade had already walked into, one release further along.
...
- Existing kernel dated 2024, package orphaned, no successor available
...
- **In-place upgrades hide vendor-kernel debt.** 22.04 → 24.04 succeeded and left the box
  looking healthy while the kernel silently fell off its support path. `lsb_release` tells
  you about userspace only.
- **`/boot` was misleading** — the real boot partition is `/media/boot` (FAT16). Critical
...
U-Boot                             U-Boot / Armbian boot config
  ↓                                  ↓
Hardkernel zImage / uInitrd        Armbian-maintained 6.6.x kernel
  ↓                                  ↓
Linux 6.1.90-21                    Ubuntu 26.04 LTS (resolute) armhf userspace
  ↓
Ubuntu 24.04 armhf
  ↓
Ubuntu 22.04 armhf (original install)
```

As reported by the board post-migration:

```
$ lsb_release -a
Distributor ID: Ubuntu
Description:    Armbian 26.8.1 resolute
Release:        26.04
Codename:       resolute
```

The board now tracks Armbian's release train (26.5.1 at flash time, 26.8.1 today) with a
maintained `6.6.x` kernel, on the same schedule as the rest of the fleet's package updates.
