...
| [SWEARENGEN-VMBR0-INTRA-BRIDGE-FORWARDING-BUG-2026-09.md](SWEARENGEN-VMBR0-INTRA-BRIDGE-FORWARDING-BUG-2026-09.md) | Resolved — and a correction. Same-host VM-to-VM TCP failure originally diagnosed as a kernel/QEMU bridge bug; actual cause was a guest configured with a /32 prefix, sending all replies via the gateway. Documents why ping worked while TCP did not, why a dual-tap capture cannot distinguish "dropped by the bridge" from "forwarded elsewhere", and why the routing workaround succeeded for the wrong reason. |
